...
const GuarantorDetails = ({ loanFormData, onGuarantorSubmit, guarantorDetails }) => {
    ...
    const handleAddGuarantor = (values, { resetForm }) => {
        ...
        const selectedMember = members.find(member => 
            member.id === values.memberId || 
            member._id === values.memberId
        );
    ...
    const handleMemberChange = (event, setFieldValue) => {
        ...
        if (selectedMember) {
            setFieldValue('membershipNumber', selectedMember.membershipNumber || selectedMember.memberId || '');
            setFieldValue('name', `${selectedMember.firstName || ''} ${selectedMember.lastName || ''}`.trim());
            setFieldValue('address', selectedMember.address || selectedMember.currentAddress || '');
        }
    };
    ...
    const getMemberDisplayName = (member) => {
        ...
        const membershipNumber = member.membershipNumber || member.memberId || '';
        
        return `${firstName} ${lastName}`.trim() + (membershipNumber ? ` - ${membershipNumber}` : '');
    };
